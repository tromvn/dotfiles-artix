...
awful.keyboard.append_global_keybindings({
	...
	end, { description = "open text editor", group = "app" }),

	--- Calculator
	awful.key({}, "XF86Calculator", function()
		awful.spawn(apps.default.calculator)
	end, { description = "open galculator", group = "app" }),
	...
	--- Poweroff
	awful.key({ mod, ctrl, shift }, "p", function()
		awful.spawn("sudo poweroff")
	end, { description = "poweroff", group = "WM" }),

	--- Reboot
	awful.key({ mod, ctrl, shift }, "b", function()
		awful.spawn("sudo reboot")
	end, { description = "reboot", group = "WM" }),

	--- Exit screen
	awful.key({ mod }, "Escape", function()
		awesome.emit_signal("module::exit_screen:show")
	end, { description = "exit screen", group = "WM" }),
})
...
awful.keyboard.append_global_keybindings({
	...
	awful.key({ mod, shift }, "0", function()
...
awful.keyboard.append_global_keybindings({
    ...
    -- Función auxiliar: busca un tag por nombre en todas las pantallas
    -- mod+número: enfoca el tag globalmente (cambia de monitor si es necesario)
    awful.key({
        modifiers = { mod },
        keygroup = "numrow",
        description = "view tag globally",
        group = "tags",
    ...
    -- mod+ctrl+número: toggle tag cross-screen
    awful.key({
        modifiers = { mod, ctrl },
        keygroup = "numrow",
        description = "toggle tag globally",
        group = "tags",
        on_press = function(index)
            local tag_name = tostring(index)
            for s in screen do
    ...
    -- mod+shift+número: mover ventana a tag global
    awful.key({
        modifiers = { mod, shift },
        keygroup = "numrow",
        description = "move focused client to tag globally",
        group = "tags",
        on_press = function(index)
...
-- Screen
awful.keyboard.append_global_keybindings({
    awful.key({ mod, ctrl }, "Tab", function()
